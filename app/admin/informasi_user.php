<?php
    ...
    // Query
    $active = mysqli_query($conn, "SELECT * FROM users_tb LEFT JOIN data_pendaftaran ON users_tb.user_id = data_pendaftaran.user_id WHERE status_user != 0 AND role != 1");
    $nonactive = mysqli_query($conn, "SELECT * FROM users_tb WHERE status_user = 0");
...
<head>
    ...
    <title>Informasi User PSB Admin</title>
                ...
                <div id="content">
                    ...
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered">
                            ...
                            <tbody>
                            <?php 
                                foreach ($nonactive as $non) : 
                            ?>
                                <tr>
                                    <td><?= $i ?></td>
                                    <td><?= $non['nama'] ?></td>
                                    <td><?= !empty($non['nik']) ? $non['nik'] : '-' ?></td>
                                    <td><?= !empty($non['email']) ? $non['email'] : '-' ?></td>
                                    <td><?= !empty($non['no_telepon']) ? $non['no_telepon'] : '-' ?></td>
                                    <td><a href="../functions.php?type=activation&user_id=<?= $non['user_id'] ?>" class="btn btn-sm btn-outline-success" onclick="return confirm('Apakah anda yakin untuk mengaktifkan user ini?');"><span><i class="fas fa-check-circle"></i></span></a></td>
                                </tr>
                            <?php
                                $i++;
                                endforeach;
                    ...
                    <hr>
                    <h4>Informasi Pendaftaran User</h4>
                    <div class="table-responsive">
                        <table class="table table-hover table-bordered">
                            <thead>
                                <tr>
